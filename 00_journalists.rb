...
# nombre de handle
number_of_H = liste.size

# le handle le plus court
def short_sort(li)
    min = li[0].length
    shorter = " "
    li.each do |elt|
...
# combien y a t il de handle contenant  5 caracteres
def five_char_sort(list)
    nombre = 0
    list.each do |elt|
        if elt.length == 5
            nombre += 1
...
def number_of_first_letter_cap(list)
    count = 0 #compteur du nombre de handle commençant pas une lettre majuscule juste après l'arobase
    list.each do |element|
      if element[0] == '@' && element[1] == element[1].upcase
        count += 1
      end 	  
    end
    return count
